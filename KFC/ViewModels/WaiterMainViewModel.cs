         ...
         using(ExcelHelper helper = new ExcelHelper())
        {
            if (helper.Open(filePath: Path.Combine(Environment.CurrentDirectory, @"C:\Users\Public\Documents\Отчёт о заказах.xlsx")))
            {
                ...
                for (int j = 0; j < month.Length; ++j)
                {
                    ...
                    /*Excel.Worksheets worksheet = application.Worksheets.Item[];
                    worksheet.Name = month[j];

                    worksheet.Cells[1] = "Номер";
                    worksheet.Cells[2] = "Дата";
                    worksheet.Cells[3] = "Цена";
                    ...
                    while (allOrders.Count > i)
                    {
                        ...
                        {
                            worksheet.Cells[1] = allOrders[i].IdOrder;
                            worksheet.Cells[2] = allOrders[i].DateAndTime.ToString("yy-MM-dd");
                            worksheet.Cells[3] = allOrders[i].Price;
                            counter++;
                        }
                        else
                        {
                    ...
                    Excel.Range sumRange = worksheet.Range[worksheet.Cells[1],
                        worksheet.Cells[2]];
                    sumRange.Merge();
                    sumRange.Value = "Итого:";

                    worksheet.Cells[3].Formula =
